...
# First pass to find the maximum number of days in any month
for month in df.groupby(df.index.month):
    num_days = len(set(month[1].index.day))
    if num_days > max_days:
        max_days = num_days

# Second pass to create the list of lists, padding with empty DataFrames
for month in df.groupby(df.index.month):
    dailyList = []
    for day in month[1].groupby(month[1].index.day):
        dailyList.append(day[1])
    
    # If this month has fewer days than the max, pad with empty DataFrames
    while len(dailyList) < max_days:
        dailyList.append(pd.DataFrame())
    
    totalList.append(dailyList)

# # Now that each month has the same number of days, we can convert to a NumPy array
# totalList = np.array(totalList)
#================================================================================================
# Find the maximum number of rows and columns
max_rows = max(df.shape[0] for month in totalList for df in month)
max_cols = max(df.shape[1] for month in totalList for df in month)

# Create a new list of lists where all DataFrames have the same shape
paddedList = []
for month in totalList:
    paddedMonth = []
    for df in month:
        df = df.reset_index(drop=True)
        padded_df = df.reindex(index=range(max_rows), columns=range(max_cols))
        paddedMonth.append(padded_df)
    paddedList.append(paddedMonth)

# Now that all DataFrames have the same shape, we can convert to a NumPy array
totalArray = np.array(paddedList)
